...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, bn_momentum, leaky_slope, dropRate):
        ...
        for i in range(nb_layers):
            layers.append(
                block(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, bn_momentum, leaky_slope,
                      dropRate))
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, bn_momentum=0.1, leaky_slope=0.0, dropRate=0.0):
        ...
        self.predictor = nn.Sequential(nn.Linear(nChannels[3], nChannels[3] // 4, bias=False),
                                        nn.BatchNorm1d(nChannels[3] // 4),
                                        nn.ReLU(inplace=True), # hidden layer
                                        nn.Linear(nChannels[3] // 4, nChannels[3])) # output layer

    def forward(self, x, ood_test=False, simsiam=False):
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
