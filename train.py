...
from datasets.data_utils import get_data_loader, make_weights_for_balanced_classes
...
def main(args):
    ...
    '''
    save_name_from_args = '{}_{}_{}'.format(args.aggregator_module, args.loss, args.num_labels)
    if args.simclr_aug:
        save_name_from_args = save_name_from_args + '_simclrAug'
    if args.simsiam:
        save_name_from_args = save_name_from_args + '_simsiam'
    if args.strong_aug:
        save_name_from_args = save_name_from_args + '_strongAug'
    if args.save_name is None:
        args.save_name = save_name_from_args
    else:
        args.save_name = args.save_name + save_name_from_args
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    optimizer = get_SGD(
        model.train_model, model.aggregator, 'SGD', args.lr, args.lr_aggregator, args.momentum, args.weight_decay)
    ...
    if not torch.cuda.is_available():
        ...
        model.aggregator = torch.nn.DataParallel(model.aggregator).cuda()
    ...
    lb_dset, ulb_dset, lb_labels = train_dset.get_ssl_dset(args.num_labels)
    
    _eval_dset = SSL_Dataset(name=args.dataset, train=False, 
                             num_classes=args.num_classes, data_dir=args.data_dir)
    eval_dset = _eval_dset.get_dset(train=False)
    
    loader_dict = {}
    dset_dict = {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset}
    label_per_class = int(args.num_labels / 10)
    class_idxs = [
        [i for i in range(label_per_class * 0, label_per_class * 1)],
        [i for i in range(label_per_class * 1, label_per_class * 2)],
        [i for i in range(label_per_class * 2, label_per_class * 3)],
        [i for i in range(label_per_class * 3, label_per_class * 4)],
        [i for i in range(label_per_class * 4, label_per_class * 5)],
        [i for i in range(label_per_class * 5, label_per_class * 6)],
        [i for i in range(label_per_class * 6, label_per_class * 7)],
        [i for i in range(label_per_class * 7, label_per_class * 8)],
        [i for i in range(label_per_class * 8, label_per_class * 9)],
        [i for i in range(label_per_class * 9, label_per_class * 10)],
    ]
    ...
    loader_dict['train_lb'] = get_data_loader(dset_dict['train_lb'],
                                              args.batch_size,
                                              data_sampler = args.lb_train_sampler,
    ...
    loader_dict['train_ulb'] = get_data_loader(dset_dict['train_ulb'],
                                               ...
                                               data_sampler = args.ulb_train_sampler,
                                               num_iters=args.num_train_iter,
    ...
    if args.resume:
        model.load_model(args.load_path, args.gpu)
        model.train_model.cuda(args.gpu)
    # START TRAINING of FixMatch
    # pretrainer = model.pretrain
    trainer = model.train
    for epoch in range(args.epoch):

        # pretrainer(args, logger=logger)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_train_iter', type=int, default=2**21, 
    ...
    parser.add_argument('--p_cutoff_paws', type=float, default=0.95)
    parser.add_argument('--p_cutoff_fix', type=float, default=0.95)
    parser.add_argument('--p_cutoff', type=float, default=0.95)
    parser.add_argument('--ema_m', type=float, default=0.999, help='ema momentum for eval_model')
    parser.add_argument('--ulb_loss_ratio', type=float, default=1.0)
    parser.add_argument('--loss', 
        choices=['PAWSLoss', 'BidirectionalLoss', 'FixMatchLoss', 'BidirectionalLoss_all', 'BidirectionalLoss_prob'],
        default='FixMatchLoss')
    parser.add_argument('--aggregator_module',
        choices=['PAWSNonParametric', 'QueuePAWSTransformer', 'PAWSTransformer', 
        'SelfNonParametric', 'SelfNonParametric_Label', 'SelfNonParametric_Prototype', 'SelfTransformer', 'Identity'],
        default='Identity')
    ...
    parser.add_argument('--lb_train_sampler', type=str, default='WeightedRandomSampler')
    parser.add_argument('--ulb_train_sampler', type=str, default='RandomSampler')
