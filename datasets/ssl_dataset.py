...
def get_transform(mean, std, size=32, train=True, simclr_aug=True, strong_aug=False):
    if train:
        s = 1
        ...
        if strong_aug:
            transformations.transforms.insert(0, RandAugment(3,5))
...
class SSL_Dataset:
    ...
    def __init__(self,
        ...
        self.eval_transform = get_transform(
            mean[name], std[name], False, simclr_aug=False, strong_aug=False)
        
        self.weak_transform = get_transform(
            mean[name], std[name], train, simclr_aug=False, strong_aug=False)
        self.transform_1 = get_transform(
            mean[name], std[name], train, simclr_aug=False, strong_aug=False)
        self.transform_2 = get_transform(
            mean[name], std[name], train, simclr_aug=simclr_aug, strong_aug=strong_aug)
    ...
    def get_dset(self, train=False, use_strong_transform=False,      # Eval Dataset
        ...
        #from SSL_Dataset's self.transform <- self.transform define when init SSL_Dataset
        transform_1 = self.eval_transform
        transform_2 = None
        data_dir = self.data_dir
        
        return BasicDataset(data, targets, num_classes, transform_1, transform_2, onehot, train)
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        lb_dset = BasicDataset(lb_data, lb_targets, num_classes, 
                               self.weak_transform, self.weak_transform, onehot, self.train)
        #use_strong_transform = True, strong_transform = Non
        ulb_dset = BasicDataset(ulb_data, ulb_targets, num_classes,
                               self.weak_transform, transform_2, onehot, self.train)
        # ulb_dset = BasicDataset(ulb_data, ulb_targets, num_classes,
        #                        transform, use_strong_transform, strong_transform, onehot)

        return lb_dset, ulb_dset, lb_targets
